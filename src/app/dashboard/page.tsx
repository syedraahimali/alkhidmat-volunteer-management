...
export default async function DashboardPage() {
  ...
  return <AppShell userEmail={user.email} isAdmin={isAdminRole(roles)}><div className="grid gap-6">
    ...
    <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm"><p className="text-sm font-medium text-brand">Volunteer impact</p><h1 className="mt-2 text-3xl font-semibold text-slate-950">Welcome{profile?.full_name ? `, ${profile.full_name}` : ""}</h1><p className="mt-2 text-sm text-slate-600">Your participation, hours, certificates, badges, and updates in one place.</p></section>
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{[{ label: "Events registered", value: summary.events_registered, Icon: CalendarCheck }, { label: "Events attended", value: summary.events_attended, Icon: Clock }, { label: "Volunteer hours", value: summary.total_hours ?? 0, Icon: Clock }, { label: "Certificates", value: summary.certificates_earned, Icon: Award }, { label: "Badges", value: summary.badges_earned, Icon: Medal }, { label: "Profile", value: profile?.status ?? "Needs setup", Icon: UserRound }].map(({ label, value, Icon }) => <Card key={label} className="p-5"><Icon className="h-5 w-5 text-brand" /><p className="mt-4 text-sm text-slate-500">{label}</p><p className="mt-1 text-2xl font-semibold text-slate-950">{String(value)}</p></Card>)}</section>
